...
public class FeatureTextBox extends TextBox {
	protected FeatureMgr featureMgr;

	private Font font = null;

	//#ifdef DLOGGING
	private Logger logger = Logger.getLogger("FeatureTextBox");
	private boolean fineLoggable = logger.isLoggable(Level.FINE);
	private boolean finestLoggable = logger.isLoggable(Level.FINEST);
	//#endif

	public FeatureTextBox(String title, String text, int maxSize,
			int constraints) {
	...
	public void init() {
		...
		//#ifdef DLOGGING
		if (fineLoggable) {logger.fine("Starting FeatureTextBox "
			//#ifdef DMIDP20
				+ super.getTitle()
			//#endif
				);}
